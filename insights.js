...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const wearableState = readWearableState();

    // Simulate getting current data from a global store or local storage
    const currentSteps = hasRecentWearableMetric('steps')
        ? wearableState.metrics.steps.value
        : Math.floor(Math.random() * 5000) + 3000; // 3000 - 8000
    const currentSpo2 = hasRecentWearableMetric('spo2')
        ? wearableState.metrics.spo2.value
        : 94 + Math.floor(Math.random() * 5); // 94 - 98
    const sleepVal = hasRecentWearableMetric('sleep')
        ? wearableState.metrics.sleep.value
        : 5 + Math.random() * 3; // 5 - 8
